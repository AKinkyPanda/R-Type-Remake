...
public:
	...
	vec2 operator / ( float operand ) const { return vec2( x / operand, y / operand ); }
	vec2 operator == ( const vec2& a ) const { if (x == a.x && y == a.y) return true; return false; }
	vec2 operator == ( float operand ) const { if (x == operand && y == operand) return true; return false; }
	void operator -= ( const vec2& a ) { x -= a.x; y -= a.y; }
	void operator += ( const vec2& a ) { x += a.x; y += a.y; }
	void operator *= ( const vec2& a ) { x *= a.x; y *= a.y; }
	void operator *= ( float a ) { x *= a; y *= a; }
	bool operator > (const vec2& a) { if (x > a.x && y > a.y) return true; return false; }
	bool operator < (const vec2& a) { if (x < a.x && y < a.y) return true; return false; }
